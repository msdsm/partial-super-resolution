...
class MyCrossAttention(nn.Module):
    def __init__(self, num_heads=4, in_dim=96, num_groups=32, embedding_dim=96, raw_img_size=64):
        super().__init__()
        self.num_heads = num_heads
        ...
        self.norm = nn.LayerNorm([raw_img_size*raw_img_size, embedding_dim])
...
class AttenBlocks(nn.Module):
    ...
    def forward(self, x, x_size, params, y):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
        y = self.my_encoder(y.view(y.size()[0], self.raw_img_size * self.raw_img_size , 1))
        x = self.my_cross_attention(x, y)
        return x
...
class RHAG(nn.Module):
    ...
    def forward(self, x, x_size, params, y):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, y), x_size))) + x
...
class PartialHAT(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(PartialHAT, self).__init__()
